...
if(!class_exists('THWDPF_Admin_Settings')):

abstract class THWDPF_Admin_Settings{
	protected $page_id = '';
	...
	public function __construct() {
		$this->tabs = array( 
			'product_rules' => __('Product Rules','discount-and-dynamic-pricing'), 
			'cart_rules'    => __('Cart Rules','discount-and-dynamic-pricing'),
			'advanced_settings'  => __('Advanced Settings','discount-and-dynamic-pricing')
			// 'premium_info'  => __('Premium Features','discount-and-dynamic-pricing')
		);
	}
	...
	public function get_admin_url($tab = false, $section = false){
		$url = 'admin.php?page=thwdpf_settings';
		if($tab && !empty($tab)){
			$url .= '&tab='. $tab;
		}
	...
	*******************************************/
	...
	public function render_form_element_tooltip($tooltip=''){
		...
		if($tooltip){
			// $icon = THWDPF_ASSETS_URL_ADMIN.'/css/help.png';
	...
	public function render_form_field_element($field, $atts = array(), $render_cell = true){
		if($field && is_array($field)){
			...
			$flabel    = isset($field['label']) && !empty($field['label']) ? __($field['label'], 'discount-and-dynamic-pricing') : '';
			$sub_label = isset($field['sub_label']) && !empty($field['sub_label']) ? __($field['sub_label'], 'discount-and-dynamic-pricing') : '';
			$tooltip   = isset($field['hint_text']) && !empty($field['hint_text']) ? __($field['hint_text'], 'discount-and-dynamic-pricing') : '';
